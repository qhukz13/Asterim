...
- **Data.** Everything is in `~/.asterim` (mode 0700; database 0600). Credentials stored by Asterim are encrypted at rest and redacted from logs. Asterim makes no outbound connections of its own; `ASTERIM_SOVEREIGN_MODE=true` turns off the frozen cloud features as well. The usage summary (`asterim stats`, and Settings in the dashboard) is computed from that database on demand, carries no name, path, prompt or command, and is transmitted nowhere — sharing it is the user's choice.
