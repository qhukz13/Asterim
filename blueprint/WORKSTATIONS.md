...
- **Zero Configuration (Deferred)**: Ideally, a developer on a laptop on the same Wi-Fi network as their desktop should open Asterim Web, enable Developer Mode, and see their Desktop automatically listed as a target workstation. *Note: Currently, mDNS discovery is deferred, and users must manually add the workstation IP and port.*
