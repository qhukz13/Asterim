...
export function Sidebar({
  ...
  const handleCreateThread = async () => {
    const name = prompt('Enter a name for the new parallel agent session:');
    if (!name) return;
  ...
  return (
    <div className="sidebar glass-panel" style={{ width: '260px', height: '100%', display: 'flex', flexDirection: 'column', borderRight: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.6)' }}>
      <div style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <button className="btn-primary" onClick={handleCreateThread} style={{ padding: '6px 12px', fontSize: '0.85rem', borderRadius: '6px' }}>
          + New Agent
        </button>
      </div>
