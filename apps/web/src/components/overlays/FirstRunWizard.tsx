...
export function FirstRunWizard({ activeBackendUrl, onComplete }: FirstRunWizardProps) {
  ...
  return (
    <div className="dialog-overlay">
      <div className="dialog-box glass-panel" style={{ maxWidth: '600px', width: '100%', padding: '32px' }}>
        {wizardStep === 1 && (
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '3.5rem', marginBottom: '16px' }}>🚀</div>
            <h1 style={{ marginBottom: '16px', fontSize: '2rem', background: 'linear-gradient(135deg, var(--color-accent-primary), var(--color-accent-hover))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Welcome to Asterim</h1>
        ...
        {wizardStep === 2 && (
          <div>
            ...
            <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.9rem', marginBottom: '24px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '32px' }}>
              <div 
                ...
                style={{
                  flex: 1,
                  padding: '16px 8px',
                  background: selectedDefaultAgent === 'claude' ? 'var(--color-accent-transparent)' : 'rgba(0,0,0,0.2)',
                  border: selectedDefaultAgent === 'claude' ? '2px solid var(--color-accent-primary)' : '1px solid var(--color-border-default)',
                  borderRadius: '16px',
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  textAlign: 'center'
              ...
              >
                ...
                <div style={{ fontSize: '0.7rem', color: 'var(--color-text-secondary)' }}>Anthropic CLI agent</div>
              </div>
              ...
              <div 
                ...
                style={{
                  flex: 1,
                  padding: '16px 8px',
                  background: selectedDefaultAgent === 'aider' ? 'var(--color-accent-transparent)' : 'rgba(0,0,0,0.2)',
                  border: selectedDefaultAgent === 'aider' ? '2px solid var(--color-accent-primary)' : '1px solid var(--color-border-default)',
                  borderRadius: '16px',
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  textAlign: 'center'
              ...
              >
                ...
                <div style={{ fontSize: '0.7rem', color: 'var(--color-text-secondary)' }}>Python Git agent</div>
              </div>
              ...
              <div 
                ...
                style={{
                  flex: 1,
                  padding: '16px 8px',
                  background: selectedDefaultAgent === 'antigravity' ? 'var(--color-accent-transparent)' : 'rgba(0,0,0,0.2)',
                  border: selectedDefaultAgent === 'antigravity' ? '2px solid var(--color-accent-primary)' : '1px solid var(--color-border-default)',
                  borderRadius: '16px',
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  textAlign: 'center'
              ...
              >
                ...
                <div style={{ fontSize: '0.7rem', color: 'var(--color-text-secondary)' }}>Google AI agent</div>
              </div>
            </div>
        ...
        {wizardStep === 3 && (
          <div>
            ...
            <div style={{ color: 'var(--color-text-secondary)', fontSize: '0.95rem', lineHeight: '1.6', marginBottom: '28px', textAlign: 'left', background: 'rgba(0,0,0,0.2)', padding: '20px', borderRadius: '12px', border: '1px solid var(--color-border-default)' }}>
            ...
            {error && <div style={{ color: 'var(--color-error-primary)', marginBottom: '16px', textAlign: 'center' }}>{error}</div>}
            
            <button onClick={handleWizardComplete} disabled={isSaving} className="btn-primary" style={{ padding: '14px 28px', width: '100%', background: 'var(--color-success-primary)' }}>
