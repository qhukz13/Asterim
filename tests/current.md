...
3. Run Team Agent Service Unit & Integration Tests:
   `pnpm --filter asterim exec tsx src/services/ai/__tests__/TeamAgentService.test.ts`
   PASS: All team agent and turn concurrency assertions pass with exit code 0.

4. Run Full Monorepo Test Battery:
   `pnpm run test`
   PASS: All 44+ test suites pass with 0 failures across 5,100+ assertions.
