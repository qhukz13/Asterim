...
- [x] **Send a task that requires a shell command. Same card. The Terminal tab is untouched.** Verified by `tools/e2e/gate-checks.mjs` (2026-09-09). The card reads "Run a shell command on this machine", quotes the agent's own description of it, and shows the exact command in mono (`docs/screenshots/gate/04-shell-approval.png`). Denied, the file that command would have created is not on disk. Worth knowing: asked to "run echo hello", Claude Code often just replies "hello" without invoking anything, so no card appears — nothing was attempted. The gate sees what the agent tries, not what it was asked.
- [ ] Transcript shows the agent's text, tool calls and results; status pill goes working → idle with cost.
- [~] **Changes tab shows the diff; commit from Changes creates the commit with the user as author; the agent has not committed.** The Changes view renders on every core-loop run. "The agent has not committed" holds by construction: `GitService` commits only on a `git.action` event, and the only thing in the repository that publishes one is the Changes view in the dashboard. No adapter can reach it. The nuance worth stating: an agent can still run `git commit` as a shell command, and that goes through the approval card like anything else. **Open:** clicking Commit in a real repository and checking the author is a founder step; the e2e project is not a git repository.
...
- [~] **Antigravity adapter carries a status word; with `MOCK_AGENT=true` the packaged binary shows the mock approval card.** It reads *preview* in the dashboard, the first-run wizard, the site's adapter table, the footer, the FAQ, the pricing list and the README. `dist/mock-antigravity.js` is in the published tarball. **Open:** selecting Antigravity and driving the mock to a card is a founder step.
...
- [x] **Landing page makes no claim that is not on this checklist. Install command is the real one. Screenshots are real captures, legible at the size shown, and regenerated from the current build.** All three product shots are e2e captures from the build in this commit, taken at 1512x900 after the dialog settles. The install command is the real one, but the package is not published yet: publishing has to happen before the site goes live, or the first command a visitor runs fails. That is P0-06 and a founder action.
...
- [x] **No simulated product UI anywhere. A diagram is labelled a diagram; a recording is labelled a recording.** The site carries exactly three images, all real screenshots, plus an inline GitHub icon. There are no mockups, no diagrams and no recordings to mislabel.
...
- [x] **`README.md` says the same thing as the landing page.** Same claims, same status words, same licence and requirements, and the same privacy statement. Two deliberate differences: the README states the longer-term vision, which the site does not; and the README offers a from-source fallback because the npm package is not published yet, while the site gives the install command unconditionally.

## E. Operations (blocking)

- [x] **`crash.log` and `server.log` land in the data directory; the diagnostic bundle copies without paths outside the data dir.** Both files were written to the channel data directory on the clean install. The bundle replaces the data directory with `<data-dir>` and the home directory with `<home>` before anything leaves the process, and drops credential-shaped strings; `AgentDiagnostics.test.ts` asserts each replacement, and the Settings screenshot shows the redacted form.
