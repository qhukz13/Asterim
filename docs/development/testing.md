...
Three more Windows defects came out of the suites that reordering finally let run, all in `VerificationPipelineService`:

- **A step that hangs never timed out.** `runStep` spawns with `shell: true`, so the child is `cmd.exe` and the real command is its grandchild. `child.kill()` is `TerminateProcess` on the shell alone, the grandchild keeps the pipes open, `close` never fires and the promise never settles — the suite hung indefinitely rather than failing. It now kills the tree with `taskkill /pid <pid> /t /f`, and a last-resort timer settles the call even if the process survives. This one was a product defect, not a test defect.
- **A killed step reported an exit code.** POSIX gives `null` for a signalled process; Windows gives whatever the terminated shell returned, usually `1`, which reads as "the command failed" rather than "we stopped it". `runStep` now reports `null` whenever it timed out, on every platform.
- **Two assertions were written for one shell.** One matched only POSIX wording for a missing binary (`cmd.exe` says "is not recognized as an internal or external command"); one compared a literal POSIX path against a message that names the resolved path, which on Windows is an absolute path under the current drive.

`AgentMcpIntegration.test.ts` now runs **last** in `apps/server`'s test chain. The chain is `&&`-joined, so a failure stops everything after it. While this suite sat in the middle, nine suites after it had never run on Windows at all, and they were hiding five defects: the three in `VerificationPipelineService` above, plus `AgentDelegationService.test.ts` and `WorktreeFleet.test.ts`, which both compared file bytes inside temporary repositories that had never been told `core.autocrlf false`. All five are fixed. Keep the known-failing suite at the end of the chain until it passes, and never let a suite that cannot pass sit in the middle of one.
